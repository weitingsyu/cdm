...
public class Program {
  ...
  public static void main(String[] args) throws ExecutionException, InterruptedException {
    ...
    cdmCorpus.getStorage().setDefaultNamespace("local"); // local is our default. so any paths that start out navigating
    ...
    CdmTypeAttributeDefinition personAttributeId = createEntityAttributeWithPurposeAndDataType(cdmCorpus,
        CUSTOM_PERSON_ENTITY_NAME + "Id", "identifiedBy", "entityId");
    personEntity.getAttributes().add(personAttributeId);
    CdmTypeAttributeDefinition personAttributeName = createEntityAttributeWithPurposeAndDataType(cdmCorpus,
        CUSTOM_PERSON_ENTITY_NAME + "Name", "hasA", "string");
    ...
    CdmDocumentDefinition personEntityDoc = cdmCorpus.makeObject(CdmObjectType.DocumentDef,
        CUSTOM_PERSON_ENTITY_NAME + ".cdm.json", false);
    ...
    // CustomPerson
    // Create the entity definition instance
    CdmEntityDefinition accountEntity = cdmCorpus.makeObject(CdmObjectType.EntityDef, CUSTOM_ACCOUNT_ENTITY_NAME,
        false);
    // Add type attributes to the entity instance
    CdmTypeAttributeDefinition accountAttributeId = createEntityAttributeWithPurposeAndDataType(cdmCorpus,
        CUSTOM_ACCOUNT_ENTITY_NAME + "Id", "identifiedBy", "entityId");
    accountEntity.getAttributes().add(accountAttributeId);
    CdmTypeAttributeDefinition accountAttributeName = createEntityAttributeWithPurposeAndDataType(cdmCorpus,
        CUSTOM_ACCOUNT_ENTITY_NAME + "Name", "hasA", "string");
    ...
    CdmEntityAttributeDefinition accountOwnerAttribute = createAttributeForRelationshipBetweenTwoEntities(cdmCorpus,
        CUSTOM_PERSON_ENTITY_NAME, "accountOwner", attributeExplanation);
    accountEntity.getAttributes().add(accountOwnerAttribute);
    // Create the document which contains the entity
    CdmDocumentDefinition accountEntityDoc = cdmCorpus.makeObject(CdmObjectType.DocumentDef,
        CUSTOM_ACCOUNT_ENTITY_NAME + ".cdm.json", false);
    // Add an import to the foundations doc so the traits about partitons will
    // resolve nicely
    accountEntityDoc.getImports().add(FOUNDATION_JSON_PATH);
    // the CustomAccount entity has a relationship with the CustomPerson entity,
    // this relationship is defined from its attribute with traits,
    ...
    CdmEntityDefinition extendedStandardAccountEntity = cdmCorpus.makeObject(CdmObjectType.EntityDef,
        EXTENDED_STANDARD_ACCOUNT, false);
    ...
    // but CreateAttributeForRelationshipBetweenTwoEntities() can show
    // more details of how to use resolution guidance to customize your data
    CdmEntityAttributeDefinition simpleCustomAccountAttribute = createAttributeForRelationshipBetweenTwoEntities(
        cdmCorpus, CUSTOM_ACCOUNT_ENTITY_NAME, "SimpleCustomAccount", attrExplanation);
    ...
    CdmDocumentDefinition extendedStandardAccountEntityDoc = cdmCorpus.makeObject(CdmObjectType.DocumentDef,
        EXTENDED_STANDARD_ACCOUNT + ".cdm.json", false);
    // Add an import to the foundations doc so the traits about partitons will
    // resolve nicely
  ...
  private static CdmEntityAttributeDefinition createSimpleAttributeForRelationshipBetweenTwoEntities(
      CdmCorpusDefinition cdmCorpus, String associatedEntityName, String foreignKeyName, String attributeExplanation) {
